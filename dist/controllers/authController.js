...
exports.refreshTokenHandler = exports.googleAuth = exports.getGoogleClientId = exports.login = exports.signup = void 0;
...
const googleOAuthClient = new google_auth_library_1.OAuth2Client(googleClientId);
...
const getGoogleClientId = async (_req, res) => {
    if (!googleClientId) {
...
const googleAuth = async (req, res) => {
    try {
        ...
        const existingResult = await postgres_1.pool.query('SELECT * FROM users WHERE email = $1', [email]);
        ...
        if (!user) {
            ...
            const usernameCheck = await postgres_1.pool.query('SELECT 1 FROM users WHERE username = $1', [finalUsername]);
        ...
        `, [
            ...
            const created = await postgres_1.pool.query('SELECT * FROM users WHERE id = $1', [userId]);
            user = created.rows[0];
        }
        else {
            ...
            if (!user.avatar_url && picture) {
                await postgres_1.pool.query('UPDATE users SET avatar_url = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [picture, user.id]);
            }
        }
