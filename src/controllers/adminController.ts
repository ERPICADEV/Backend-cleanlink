...
export const assignReport = async (req: Request, res: Response) => {
  try {
    ...
    const adminResult = await pool.query('SELECT id FROM admins WHERE id = $1', [assigned_to])
    const targetAdmin = adminResult.rows[0] as any
    ...
    const progressCheckResult = await pool.query('SELECT id FROM report_progress WHERE report_id = $1', [id])
    ...
    if (existingProgress) {
      ...
      `, [randomUUID(), id, assigned_to, notes || ''])
    }
...
export const resolveReport = async (req: Request, res: Response) => {
  try {
    ...
    // Get comments count
    const commentsResult = await pool.query('SELECT COUNT(*) as count FROM comments WHERE report_id = $1', [id])
...
export const getReportAuditLogs = async (req: Request, res: Response) => {
  try {
    ...
    const auditLogs = auditResult.rows as any[]
...
export const getAdminUsers = async (req: Request, res: Response) => {
  try {
    ...
    const admins = result.rows as any[]
...
export const getAdminStats = async (req: Request, res: Response) => {
  try {
    ...
    let resolvedResult
    if (adminRegion) {
      resolvedResult = await pool.query(`
        SELECT COUNT(*) as count FROM reports 
    ...
    let avgTimeResult
    if (adminRegion) {
      avgTimeResult = await pool.query(`
        SELECT 
          AVG(
    ...
    const categoryResult = await pool.query(`
      ...
      LIMIT 5
    `)
    const byCategory = categoryResult.rows as any[]
    ...
    const recentResult = await pool.query(`
      ...
      LIMIT 5
    `)
    const recentActivity = recentResult.rows as any[]
...
export const updateReportProgress = async (req: Request, res: Response) => {
  try {
    ...
    if (progress_status === 'in_progress') {
      await pool.query('UPDATE reports SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', ['in_progress', reportId])
    }
...
export const getPendingApprovals = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(convertedSql, [parseInt(limit as string)])
