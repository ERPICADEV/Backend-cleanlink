...
export const updateMe = async (req: Request, res: Response) => {
  try {
    ...
    if (username) {
      const existingResult = await pool.query('SELECT id FROM users WHERE username = $1 AND id != $2', [username, req.userId]);
    ...
    `, [req.userId]);
    const updated: any = userResult.rows[0];
...
export const getMyComments = async (req: Request, res: Response) => {
  try {
    ...
    // Get total count
    const countResult = await pool.query('SELECT COUNT(*) as count FROM comments WHERE author_id = $1', [userId]);
...
export const updateRegion = async (req: Request, res: Response) => {
  try {
    ...
    await pool.query('UPDATE users SET region = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [JSON.stringify(region), req.userId]);

    // Get updated user
    const userResult = await pool.query('SELECT id, username, email, region FROM users WHERE id = $1', [req.userId]);
    const updated: any = userResult.rows[0];
