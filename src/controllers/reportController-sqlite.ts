...
export const getReport = async (req: Request, res: Response) => {
  try {
    ...
    let isAdmin = false;
    if (req.userId) {
      const adminStmt = db.prepare('SELECT 1 FROM admins WHERE user_id = ?');
    ...
    `);
    const rawComments = commentsStmt.all(id) as any[];
    ...
    const formatComment = (comment: any): any => {
      ...
      const repliesStmt = db.prepare(`
        ...
        ORDER BY c.created_at ASC
      `);
      const replies = repliesStmt.all(comment.id) as any[];
    ...
    const comments = topLevelComments.map((comment: any) => formatComment(comment));
...
export const updateReport = async (req: Request, res: Response) => {
  try {
    ...
    const existingReport = getStmt.get(id) as any;
    ...
    // Get updated report
    const updatedReportStmt = db.prepare('SELECT * FROM reports WHERE id = ?');
    const updatedReport = updatedReportStmt.get(id) as any;
