...
export const redeemReward = async (req: Request, res: Response) => {
    try {
      ...
      const userResult = await pool.query('SELECT id, civic_points FROM users WHERE id = $1', [req.userId!]);
      ...
      const redemptionCountResult = await pool.query('SELECT COUNT(*) as count FROM redemptions WHERE user_id = $1 AND reward_id = $2', [req.userId!, id]);
      const redemptionCount = { count: parseInt(redemptionCountResult.rows[0].count) };
  
      // Get reward details
      const rewardResult = await pool.query('SELECT * FROM rewards WHERE id = $1', [id]);
...
export const createReward = async (req: Request, res: Response) => {
  try {
    ...
    const existingResult = await pool.query('SELECT id FROM rewards WHERE key = $1', [key]);
...
export const updateReward = async (req: Request, res: Response) => {
  try {
    ...
    // Get existing reward
    const existingResult = await pool.query('SELECT * FROM rewards WHERE id = $1', [id]);
    const existingReward = existingResult.rows[0] as any;
